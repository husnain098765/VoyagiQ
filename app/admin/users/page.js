  ...
  useEffect(() => {
    async function fetchUsers() {
      try {
        ...
        setError("Failed to load users data. Please ensure the /api/admin/users GET route is set up correctly.");
      } finally {
        setLoading(false);
      }
    }
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user._id} className="border-b hover:bg-gray-50 transition">
                  <td className="p-3">{user._id.toString()}</td>
                  <td className="p-3">
...
// NOTE: For the fetch calls above to work, you will still need /api/admin/users and /api/admin/users/[id] API routes.
// The GET route for /api/admin/users to fetch initial data.
// The PUT route for /api/admin/users/[id] to update a user.
// The DELETE route for /api/admin/users/[id] to delete a user.